...
const search = ref('')
const savingOwner = ref<string | null>(null)
const { data, status, refresh } = useFetch('/api/recruitment/overview', {
  key: 'pds-recruitment-overview',
  headers: useRequestHeaders(['cookie']),
})
const { data: recruiterData } = useFetch('/api/recruitment/recruiters', {
...
async function setOwner(jobId: string, ownerUserId: string) {
  savingOwner.value = jobId
  try {
    await $fetch(`/api/jobs/${jobId}/recruitment-owner`, { method: 'PUT', body: { ownerUserId: ownerUserId || null } })
    await refresh()
    toast.success(ownerUserId ? 'Requirement owner updated' : 'Requirement owner removed')
  } catch (err: any) {
    toast.error('Could not update owner', { message: err?.data?.statusMessage ?? err?.message })
  } finally { savingOwner.value = null }
}
</script>
  ...
  <div class="mx-auto max-w-7xl">
    <header class="mb-5 flex flex-wrap items-start justify-between gap-3">
      <div>
        ...
        <p class="mt-1 text-sm text-surface-500">Requirement health, ownership, candidate workload and pending recruiter actions across the organisation.</p>
      </div>
      <div class="flex gap-2">
        <NuxtLink :to="localePath('/dashboard/recruitment/workload')" class="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-3 py-2 text-sm font-semibold text-white"><UsersRound class="size-4" />Recruiter Workload</NuxtLink>
    ...
    <template v-else>
      <div class="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-7">
        ...
        <div class="rounded-xl border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900"><div class="flex items-center gap-2 text-warning-600"><AlertTriangle class="size-4" /><span class="text-xs uppercase">Unowned Jobs</span></div><p class="mt-2 text-2xl font-bold">{{ data?.summary?.unownedRequirements ?? 0 }}</p></div>
        ...
        <div class="rounded-xl border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900"><div class="flex items-center gap-2 text-warning-600"><AlertTriangle class="size-4" /><span class="text-xs uppercase">Unassigned</span></div><p class="mt-2 text-2xl font-bold">{{ data?.summary?.unassignedCandidates ?? 0 }}</p></div>
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody class="divide-y divide-surface-100 dark:divide-surface-800">
            <tr v-for="row in filtered" :key="row.id" class="hover:bg-surface-50 dark:hover:bg-surface-800/30">
              ...
              <td class="px-4 py-3 min-w-[220px]"><select :value="row.ownerUserId ?? ''" :disabled="savingOwner === row.id" class="w-full rounded-lg border border-surface-300 bg-white px-2 py-1.5 text-xs dark:border-surface-700 dark:bg-surface-900" @change="setOwner(row.id, ($event.target as HTMLSelectElement).value)"><option value="">Unassigned</option><option v-for="person in recruiterData?.recruiters ?? []" :key="person.id" :value="person.id">{{ person.name }}</option></select></td>
              <td class="px-4 py-3 font-semibold">{{ row.totalCandidates }}</td><td class="px-4 py-3" :class="row.unassignedCandidates ? 'font-semibold text-warning-600' : ''">{{ row.unassignedCandidates }}</td><td class="px-4 py-3">{{ row.assessed }}</td><td class="px-4 py-3">{{ row.screening }}</td><td class="px-4 py-3">{{ row.hod }}</td><td class="px-4 py-3">{{ row.offer }}</td><td class="px-4 py-3 font-semibold">{{ row.pendingActions }}</td>
            ...
            <tr v-if="!filtered.length"><td colspan="11" class="px-4 py-10 text-center text-surface-400">No matching active requirements.</td></tr>
          </tbody>
        </table>
      </div>
    </template>
