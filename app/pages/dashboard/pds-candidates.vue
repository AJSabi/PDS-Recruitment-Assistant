  ...
  <div class="mx-auto max-w-7xl">
    ...
    <template v-else>
      <div class="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <button class="rounded-xl border border-surface-200 bg-white p-4 text-left dark:border-surface-800 dark:bg-surface-900" @click="filter = 'with_resume'"><div class="flex items-center gap-2 text-surface-500"><FileText class="size-4" /><span class="text-xs">Resume Available</span></div><p class="mt-2 text-2xl font-bold">{{ data?.summary?.withResume ?? 0 }}</p></button>
        ...
        <button class="rounded-xl border border-surface-200 bg-white p-4 text-left dark:border-surface-800 dark:bg-surface-900" @click="filter = 'database_only'"><div class="flex items-center gap-2 text-surface-500"><UsersRound class="size-4" /><span class="text-xs">Database Only</span></div><p class="mt-2 text-2xl font-bold">{{ data?.summary?.databaseOnly ?? 0 }}</p></button>
      ...
      <div class="mb-4 flex flex-wrap gap-2">
        <div class="flex min-w-[280px] flex-1 items-center rounded-lg border border-surface-300 bg-white px-3 dark:border-surface-700 dark:bg-surface-900"><Search class="size-4 text-surface-400" /><input v-model="search" class="w-full bg-transparent px-2 py-2.5 text-sm outline-none" placeholder="Search candidate, AI summary, requirement, recruiter, status, fit or priority" /></div>
