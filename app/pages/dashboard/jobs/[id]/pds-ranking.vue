...
import { AlertTriangle, ArrowLeft, CheckCircle2, ClipboardList, FileSearch, RefreshCw, UsersRound } from 'lucide-vue-next'
    ...
    <template v-else>
      <div class="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-xl border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900">
          <div class="flex items-center gap-2 text-surface-500"><UsersRound class="size-4" /><span class="text-xs font-medium uppercase tracking-wide">Candidates</span></div>
          <p class="mt-2 text-2xl font-bold text-surface-900 dark:text-surface-50">{{ summary.total }}</p>
        </div>
        <div class="rounded-xl border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900">
          <div class="flex items-center gap-2 text-success-600"><CheckCircle2 class="size-4" /><span class="text-xs font-medium uppercase tracking-wide">Resume Assessed</span></div>
          <p class="mt-2 text-2xl font-bold text-surface-900 dark:text-surface-50">{{ summary.assessed }}</p>
        </div>
        <div class="rounded-xl border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900">
          <div class="flex items-center gap-2 text-surface-500"><ClipboardList class="size-4" /><span class="text-xs font-medium uppercase tracking-wide">Not Yet Assessed</span></div>
          <p class="mt-2 text-2xl font-bold text-surface-900 dark:text-surface-50">{{ summary.notYetAssessed }}</p>
        </div>
        <div class="rounded-xl border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900">
          <div class="flex items-center gap-2 text-warning-600"><AlertTriangle class="size-4" /><span class="text-xs font-medium uppercase tracking-wide">Reassessment Required</span></div>
          <p class="mt-2 text-2xl font-bold text-surface-900 dark:text-surface-50">{{ summary.needsReassessment }}</p>
        </div>
      </div>
