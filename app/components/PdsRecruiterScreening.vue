...
async function generateQuestions() {
  ...
  try {
    ...
    toast.success('AI screening questions generated', { message: `${generatedQuestions.value.length} questions ready for recruiter review.` })
  } catch (err: any) {
...
async function startScreening(useAi = false) {
  ...
  if (!qs.length) return toast.warning('Add screening questions', 'Generate AI questions or enter one question per line.')
  busy.value = true
  try {
...
<template>
  <section class="rounded-xl border border-surface-200 bg-white p-5 dark:border-surface-800 dark:bg-surface-900">
    <div class="mb-4"><h2 class="text-sm font-semibold text-surface-800 dark:text-surface-200">Recruiter Screening</h2><p class="mt-1 text-xs text-surface-500">AI generates candidate-specific questions from the JD, approved Skill Matrix, resume assessment and verification gaps. The recruiter still conducts and confirms the screening.</p></div>

    <div v-if="!enabled && screening?.status === 'not_started'" class="rounded-lg border border-dashed border-surface-300 p-4 text-sm text-surface-500 dark:border-surface-700">Complete the resume assessment before starting recruiter screening.</div>
    ...
    <div v-else-if="screening?.status === 'not_started'" class="space-y-4">
      <div class="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-brand-200 bg-brand-50/50 p-4 dark:border-brand-900 dark:bg-brand-950/20">
        <div><p class="text-sm font-semibold">AI Screening Questions</p><p class="mt-1 text-xs text-surface-500">Recommended route for candidate-specific recruiter screening.</p></div>
        <button :disabled="aiGenerating || busy" class="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50" @click="generateQuestions"><Loader2 v-if="aiGenerating" class="size-4 animate-spin" /><Sparkles v-else class="size-4" />{{ aiGenerating ? 'Generating…' : (generatedQuestions.length ? 'Regenerate Questions' : 'Generate AI Questions') }}</button>
      </div>
      ...
      <div v-if="generatedQuestions.length" class="space-y-2">
        ...
        <div class="flex justify-end"><button :disabled="busy" class="rounded-lg bg-success-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50" @click="startScreening(true)">Start Screening with AI Questions</button></div>
      </div>

      <details class="rounded-lg border border-surface-200 p-4 dark:border-surface-800"><summary class="cursor-pointer text-sm font-medium">Manual questions instead</summary><div class="mt-3 space-y-3"><textarea v-model="questionText" rows="6" class="w-full rounded-lg border border-surface-300 px-3 py-2 text-sm dark:border-surface-700 dark:bg-surface-800" placeholder="Enter one question per line. Up to 10 questions." /><div class="flex justify-end"><button :disabled="busy" class="rounded-lg border border-surface-300 px-4 py-2 text-sm font-semibold disabled:opacity-50" @click="startScreening(false)">Start Manual Screening</button></div></div></details>
    </div>
