...
export default defineEventHandler(async (event) => {
  ...
  if (current.googleCalendarEventId) {
    cancelCalendarEvent(current.createdById, current.googleCalendarEventId).catch(err => {
      logError('calendar.cancel_event_on_delete_failed', { event_id: current.googleCalendarEventId, error_message: err instanceof Error ? err.message : String(err) })
    })
  }

  await db.delete(interview).where(and(eq(interview.id, id), eq(interview.organizationId, orgId)))
  recordActivity({ organizationId: orgId, actorId: session.user.id, action: 'deleted', resourceType: 'interview', resourceId: id })
