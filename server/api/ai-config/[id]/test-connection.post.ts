...
export default defineEventHandler(async (event) => {
  ...
  const config = await db.query.aiConfig.findFirst({ where: and(eq(aiConfig.id, id), eq(aiConfig.organizationId, orgId)) })
  if (!config) throw createError({ statusCode: 404, statusMessage: 'AI configuration not found.' })

  try {
    await generateStructuredOutput(
      { provider: config.provider as SupportedProvider, model: config.model, apiKeyEncrypted: config.apiKeyEncrypted, baseUrl: config.baseUrl, maxTokens: 20 },
