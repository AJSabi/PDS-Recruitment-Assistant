...
export default defineEventHandler(async (event) => {
  ...
  if (body.questionId !== nextQuestion.id) throw createError({ statusCode: 422, statusMessage: 'Answer the current screening question before moving to the next question.' })
  ...
  const [updated] = await db.update(recruiterScreeningSession).set({ responses: updatedResponses, updatedAt: new Date() }).where(eq(recruiterScreeningSession.id, screening.id)).returning()
  const answered = updatedResponses.length

  return { screening: updated, currentQuestion: questions[answered] ?? null, readyToComplete: answered === questions.length, progress: { answered, total: questions.length } }
})
