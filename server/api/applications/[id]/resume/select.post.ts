...
export default defineEventHandler(async (event) => {
  ...
  if (!resume) {
    throw createError({ statusCode: 404, statusMessage: 'Resume document not found for this candidate' })
  }
  ...
  if (!allowedStatuses.has(profile.lastStatus)) {
    throw createError({
      statusCode: 422,
      statusMessage: `Resume selection cannot change while candidate status is ${profile.lastStatus}. Use Reassess first if a new resume must replace the assessed baseline.`,
    })
  }
  ...
  const nextAction = profile.lastStatus === 'reassess'
    ? 'Complete reassessment using the selected resume.'
    : 'Complete resume assessment against the approved requirement baseline.'
