...
describe('PDS full-flow audit safeguards', () => {
  it('keeps the PDS-facing candidate record free of legacy application, interview and shared-candidate deletion bypasses', () => {
    ...
    expect(page).not.toContain('deleteCandidate')
    expect(page).not.toContain('Delete candidate?')
    expect(page).toContain('/dashboard/recruitment/${app.id}')
    expect(page).toContain("requirement's JD & Skill Matrix / Add Candidate flow")
    expect(page).toContain('Candidate deletion/retention is governed')
  })
  ...
  it('scopes Candidate Database application counts to visible requirements for recruiters', () => {
    const api = source('server/api/candidates/index.get.ts')
    expect(api).toContain('getRequirementVisibility')
    expect(api).toContain('visibility.canSeeAll')
    expect(api).toContain('recruitmentRequirementState.ownerUserId, session.user.id')
