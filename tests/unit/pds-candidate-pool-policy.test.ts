...
describe('PDS Candidate Pool scoring policy', () => {
  ...
  it('makes Mandatory evidence the dominant scoring dimension', () => {
    const result = calculateProvisionalFit({
      mandatoryScore: 100,
      preferredScore: 0,
      experienceScore: 0,
      optionalScore: 0,
  ...
  it('downgrades candidates with critical Mandatory weakness even when other dimensions are strong', () => {
    const criticalGap = calculateProvisionalFit({
      mandatoryScore: 39,
      preferredScore: 100,
      experienceScore: 100,
      optionalScore: 100,
    ...
    const materialGap = calculateProvisionalFit({
      mandatoryScore: 55,
      preferredScore: 100,
      experienceScore: 100,
      optionalScore: 100,
...
describe('PDS Candidate Pool visibility and authorization policy', () => {
  ...
  it('persists full AI assessments before deciding whether a candidate is hidden below 50 percent', () => {
    ...
    expect(source).toContain('MAX_FULL_AI_ANALYSES_PER_SYNC = 30')
  })
})
