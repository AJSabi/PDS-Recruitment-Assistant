...
p = Path('app/components/ApplyCandidateModal.vue')
s = p.read_text()
...
def repl(old: str, new: str, count: int = 1) -> None:
    ...
    actual = s.count(old)
    if actual != count:
        raise SystemExit(f'Expected {count} occurrence(s), found {actual} for: {old[:100]!r}')
...
test = Path('tests/unit/pds-resume-identity-review-ux.test.ts')
